...
mpd_sync() {
    ...
    if ! cp -r "${MPD_DOTFILES_PATH_REPO}"/* "${MPD_CONFIG_PATH}"; then
...
ncmpcpp_sync() {
    ...
    if ! cp -r "${ncmpcpp_DOTFILES_PATH_REPO}"/* "${NCMPCPP_CONFIG_PATH}"; then
...
audio_sync() {
    ...
    ausy_builder() {
        ...
        case "not" in
        "*${AUSY_ROP}*")
            echo "|> [PASS]: did find the string [NOT] at the [AUSY_ROP] array. Proceeding with the error railway... "
            return 1
            ;;
        *) ;;
        esac
